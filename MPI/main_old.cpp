...
static vector<int> build_row_partition_by_nnz(const CSRMatrix& A, int P)
{
    ...
    //total contains total number of non-zero elements
    long long total = 0;
    for (auto v : row_nnz) total += v;

    // Compute the target number of non-zeros per process
    ...
    //acc represents accumulated number of non-zero elements assigned so far
    long long acc = 0;
    int cur_row = 0;
    for (int r = 1; r < P; ++r) {
        //goal represents the target number of non-zero elements for this process
...
int mpi(int argc, char** argv)
{
    ...
    // Determine local matrix dimensions and offsets
